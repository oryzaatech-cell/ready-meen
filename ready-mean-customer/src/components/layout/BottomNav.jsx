              ...
              <>
                <div className={`relative p-1.5 rounded-xl transition-all duration-300 ${isActive ? 'bg-primary-600 shadow-md shadow-primary-600/25' : ''}`}>
                  ...
                  {badgeKey === 'notifications' && unreadCount > 0 && (
                    <span className={`absolute -top-1.5 -right-1.5 text-white text-[8px] font-bold rounded-full min-w-[16px] h-4 flex items-center justify-center px-1 border-2 border-white shadow-sm ${isActive ? 'bg-amber-500' : 'bg-red-500'}`}>
