...
export function AuthProvider({ children }) {
  ...
  const signUp = async (mobile, password, metadata = {}) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL || '/api'}/auth/signup`, {
      ...
      body: JSON.stringify({ ...metadata, mobile, password, role: 'vendor' }),
    });
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (profileData.user?.role === 'customer') {
            ...
            throw new Error('This account is registered as a Customer.');
          }
        }
      } catch (err) {
        if (err.message.includes('Customer')) throw err;
  ...
  const signOut = async () => {
    ...
    localStorage.removeItem('vendor_orders_last_seen');
    localStorage.removeItem('vendor_onboarding_done'); // legacy cleanup
